...
export const client = {
    ...
    async createOrder(order) {
        ...
        const consumerSecret = "cs_661249c3135e6b9d86ae3fd7fae5a94bbc624e9e";
        const encodedCredentials = btoa(`${consumerKey}:${consumerSecret}`);
        const userEmail = order[0].billing.email;
        const getEmailUrl = `https://rejuve.com/wp-json/custom/v1/get-user-id?email=${userEmail}`;

        let currentUserId = 0;

        const currentUserIdResponse = await fetch(getEmailUrl, {
            headers: {
                Authorization: `Basic ${encodedCredentials}`,
                "Content-Type": "application/json"
            },
        ...
        currentUserId = currentUserIdData.user_id || 0;
        ...
        try {
            ...
            const requests = validOrders.map(async (singleOrder) => {
                const response = await fetch(url, {
                    ...
                    body: JSON.stringify({ ...singleOrder, customer_id: currentUserId })
                });
